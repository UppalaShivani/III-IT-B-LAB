DROP DATABASE IF EXISTS `19wh1a1279_ticket_reservation`;
CREATE SCHEMA 19wh1a1279_ticket_reservation;
USE 19wh1a1279_ticket_reservation;
CREATE TABLE pasangers(pid int NOT NULL, name VARCHAR(30),  phone_number BIGINT, address VARCHAR(30),gender VARCHAR(3), age int, PRIMARY KEY(pid) );
INSERT INTO pasangers(pid, name, phone_number, gender, address, age) VALUES(1, 'Shivani', 1234567890, 'F', 'Siddipet', 19);
...
CREATE TABLE bus(bid VARCHAR(10) NOT NULL , bcolor VARCHAR(10), from_place VARCHAR(20), destiny VARCHAR(20),n_days INT, PRIMARY KEY (bid));
...
CREATE TABLE ticket(ticket_num int NOT NULL, from_place VARCHAR(20), destiny VARCHAR(20),cost int, seat_no INT, bid int,pid int);
...
SELECT p.pid FROM pasangers p  WHERE p.pid <>ALL(SELECT t.pid FROM ticket t);                       -- <> not equal

SELECT p.pid FROM pasangers p  WHERE p.pid = ANY(SELECT t.pid FROM ticket t);

SELECT p.pid FROM pasangers p  WHERE p.pid  IN (SELECT t.pid FROM ticket t);

--   views
-- ---------
CREATE VIEW Pass_View AS SELECT * FROM pasangers;
SELECT pid, name FROM Pass_View WHERE address = 'Hyderabad';
 
CREATE VIEW Ticket_View AS SELECT ticket_num FROM ticket;
SELECT * FROM Ticket_View;

CREATE VIEW Cancel_View AS SELECT * FROM cancellation;
SELECT * FROM Cancel_View;

CREATE VIEW Bus_View AS SELECT * FROM bus;
UPDATE Bus_View SET bid='TS123' WHERE destiny = 'Hyderabad';
